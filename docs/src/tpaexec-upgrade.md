...
1.  **Start:** `PGD 5.9+` with `pgd-proxy`
2.  **Intermediate:** `PGD 5.9+` with `Connection Manager`
3.  **Final:** `PGD 6` with `PGD-X Architecture`
...
-   **Cluster Version:** Your cluster must be running `PGD` version 5.9 or
    ...
    to upgrade to the latest minor version first.
...
will fully support lifecycle management of `PGD 5` with `Connection
    Manager`.
...
Run the following command to update your `config.yml` file. This
command tells `TPA` to generate a configuration that enables the
built-in `Connection Manager`.

Before writing the new `config.yml`, `reconfigure` saves a backup of
the current file with a numbered suffix (e.g., `config.yml.~1~`),
providing a safe restore point.
...
At the end of this stage, you will have a `PGD 5.9+` cluster running
with the built-in `Connection Manager`. You can manage its
configuration with `tpaexec deploy`, but remember that `tpaexec upgrade`
is **not supported** for this intermediate state.
